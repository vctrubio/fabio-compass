...
import { StudentWelcomeForm, LanguagesEnum } from "@/rails/model";
import { toast } from "sonner";
import { z } from "zod";
import { createStudentFromClient, createStudentWithInvitation } from "@/actions/user-student-teacher-actions";
...
interface FormFieldDescriptor {
    key: keyof StudentCreate4AdminFormData;
    label: string;
...
const FORM_FIELDS: FormFieldDescriptor[] = [
    ...
    { key: 'age', label: 'Age', type: 'number', required: false, placeholder: '25' },
    { key: 'weight', label: 'Weight (kg)', type: 'number', required: false, placeholder: '70' },
    { key: 'height', label: 'Height (cm)', type: 'number', required: false, placeholder: '175' },
] as const;

export const StudentCreate4Admin = ({
    onSubmit
}: {
    ...
    const [formData, setFormData] = useState<Partial<StudentCreate4AdminFormData>>({
        name: '',
        passport_number: '',
        age: null,
        weight: null,
    ...
    const handleInputChange = (key: keyof StudentCreate4AdminFormData, value: any) => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const data: StudentCreate4AdminFormData = {
            ...
            const validation = StudentWelcomeForm.safeParse(data);
            ...
            if (email && email.trim() !== "") {
                ...
                const result = await createStudentWithInvitation(null, validatedData, email, 'pendingStudent');
                if (!result.success) {
                    throw new Error(result.error || 'Failed to create student with invitation');
                }
                newStudent = result.data?.student;
                toast.success(`Student "${validatedData.name}" created with email invitation!`, {
                    description: `Student ID: ${newStudent?.id} | Email: ${email}`,
                    duration: 4000,
                });
            } else {
                // Create student without invitation
                const result = await createStudentFromClient(validatedData);
                if (!result.success) {
                    throw new Error(result.error || 'Failed to create student');
                }
                newStudent = result.data;
                toast.success(`Student "${validatedData.name}" created successfully!`, {
                    description: `Student ID: ${newStudent?.id}`,
                    duration: 4000,
                });
            }

            resetForm();
            if (onSubmit) onSubmit(newStudent);

        } catch (err: any) {
            console.error('Error creating student:', err);
            const errorMessage = err.message || "Failed to create student";
            setError(errorMessage);
            toast.error("Error Creating Student", { description: errorMessage, duration: 4000 });
        } finally {
            setIsLoading(false);
        }
    };
                ...
                <CardHeader className="pb-4">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center">
                            <ENTITY_CONFIGS.students.icon className="h-4 w-4 text-primary" />
                        </div>
                ...
                <CardContent>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex flex-wrap gap-4">
                            ...
                            {/* Phone Input - simplified without country selection */}
