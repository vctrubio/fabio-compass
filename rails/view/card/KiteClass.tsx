...
export function KiteClass({
    event
}: { event: KiteEventData }) {
    const { id, time, duration, location, students, status } = event;
    ...
    return (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-3 space-y-2 h-full flex flex-col justify-between shadow-sm">
            ...
            {/* Footer with Status and Dropdown */}
            <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-700/50 mt-auto">
                                ...
                                <div className="p-2">
                                    ...
                                    <button
                                        onClick={async () => {
                                            setIsLoading(true);
                                            try {
                                                const result = await updateKiteEventTime(id, editedTime);
                                                if (!result.success) {
