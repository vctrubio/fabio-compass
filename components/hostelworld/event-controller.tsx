...
import {
    ...
    updateKiteEventsWithCalculatedTimeAction,
...
const SelectedKiteEventsDisplay = ({
    ...
    // State to manage expanded/collapsed teacher rows
        ...
        try {
            ...
            Object.entries(recalculatedEventsByTeacher).forEach(
                ([teacherId, teacherData]) => {
                    console.log(
                        `👨‍🏫 Processing teacher ${teacherId.slice(-6)} with ${teacherData.events.length} events:`,
                    );

                    teacherData.events.forEach((event, index) => {
                        // Get original event to compare
                        const originalEvent = todayKiteEvents?.find(
                            (e) => e.id === event.id,
                        );
                        const originalTime = originalEvent
                            ? new Date(originalEvent.date).toLocaleTimeString("es-ES", {
                                hour: "2-digit",
                                minute: "2-digit",
                                hour12: false,
            ...
            const totalGaps = Object.values(eventGaps).reduce(
                (sum, gap) => sum + gap,
                0,
            );
            if (totalGaps > 0) {
                const gapsCount = Object.keys(eventGaps).filter(
                    (id) => eventGaps[id] > 0,
                ).length;
                console.log(
                    `🕐 Total gaps applied: ${gapsCount} gap${gapsCount !== 1 ? "s" : ""} adding ${Math.floor(totalGaps / 60)}h ${totalGaps % 60}m`,
                );
            }

            console.log(
                `🔄 Updating ${eventsToUpdate.length} events with new times and details`,
            );
    ...
    return (
        <div className="mt-4">
            ...
            {Object.entries(eventsByTeacher).map(([teacherId, teacherData]) => {
                ...
                // Only render if there are events for this teacher in either original or recalculated
                if (originalEvents.length === 0 && recalculatedEvents.length === 0) {
                    return null;
                }

                const isExpanded = expandedTeachers[teacherId] ?? true; // Default to expanded
